...
// import { setTeamMember, clearTeamMember } from "../TeamMembersSlice";
import { v4 as uuidv4 } from 'uuid';
...
const CourseTeamMembers: React.FC = () => {
    ...
    // const teamCourseId = location.state.courseId || {};
    ...
    const filteredSuggestions = teamList.filter((member) => {
        const isNameMatch = member.name?.toLowerCase().includes(memberName.toLowerCase());
        const isProjectMatch = (member.project_id?.includes(project.projectId));
        const isCourseNotAssigned = !(member.course_id?.includes(courseId))
    
        return isNameMatch && isProjectMatch && isCourseNotAssigned;
    ...
    const handleFormSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (isDefaultValue?.userInviteId) {
                ...
                await axiosInstance.put(updateUrl, newMember, {
                    headers: {
            ...
            toast.error("User is not active. Please activate the user.");
        }
    };
    ...
    //console.log("courseIdTeamCourseId", teamCourseId);
    ...
    //const filteredTeamList = teamList.filter(member => member.course_id === teamCourseId);
    //const filteredTeamList = teamList.filter(member => String(member.course_id) === String(teamCourseId));
    const filteredTeamList = teamList.filter(member =>
        Array.isArray(member.course_id) &&
        member.course_id.includes(courseId)
      );

    console.log("filter list",filteredTeamList )
                    ...
                    {filteredTeamList.map((item) => {
                        ...
                        return (
                            <div key={item.id} className="flex items-center mb-3 bg-white p-3 rounded-lg text-sm text-gray-700 border border-gray-200 shadow-lg" style={{ width: "", height: "68px" }}>
                                ...
                                <button onClick={() => handleDeleteCourseMembers(item.userInviteId)}
...
export default CourseTeamMembers;
